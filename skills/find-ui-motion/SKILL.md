---
name: find-ui-motion
description: Find UI motion for Web and mobile, including 网页动效、App 动效、交互动效、微动效和动画灵感. Use when a user wants to 找、搜索、推荐、比较、预览、复制、复刻、还原或实现界面动画; mentions hover, gesture, transition, navigation, scroll, loading, text animation, page or screen entrance, CSS, JavaScript, React, Vue, SwiftUI, Jetpack Compose, Flutter, React Native, Lottie, or Rive; asks to see real source examples; has only a vague motion idea; asks for 动效网站或动画参考; or provides a webpage, app screenshot, link, image, or video as a motion reference. Guide vague requests through questions, run deterministic taxonomy and full-index fuzzy retrieval with bounded synonym expansion before Web supplementation, show live-verified source evidence during deeper review, then deliver a platform-compatible public snippet, package integration, licensed asset, or independent recreation. Prioritize code-backed, target-compatible cases; search video-only cases only after explicit user authorization. Do not use for static UI design or video editing.
---
...
```bash
python3 scripts/search_catalog.py "<user request>" --mode quick --quick-count 15 --formal-target 8 --strategy auto --candidate-limit 48 --trace
```
...
In quick mode, let `auto` combine taxonomy recall with one full-index fuzzy scan over titles, keywords, tags, targets, triggers, and motion metadata. Run bundled bilingual synonym expansion only when fewer than ten meaningful quick candidates survive, no `strong` candidate exists, or no candidate covers at least half of the explicitly requested core groups. Quick discovery uses `quick_tier=strong | related | exploratory | off-target`: stronger matches come first, while related mechanisms and high-quality exploratory directions remain visible. Style, feeling, intensity, and platform terms rank or label candidates without becoming hard initial-screen requirements. Keep the stricter `coverage=exact | adjacent | gap` for deep formal matching.
...
- Use bundled catalog metadata and cached health fields for this speed-first pass; do not open and live-verify every page before delivery. Never claim current verification from `last_shallow_check`, `last_verified`, or a catalog record. Apply [source-health.md](references/source-health.md) later to selected cases and deep-match batches.
- Show about fifteen concrete case links by default and accept an explicit quick count up to twenty. Put stronger matches first and aim for a useful mix of roughly five `strong`, six `related`, and four `exploratory` cases when the pool supports it; do not force the ratio when a tier is sparse. Reduce below fifteen only after the full fuzzy scan, and bilingual expansion when fewer than ten remain or the core behavior is still missing, cannot supply enough meaningful unique cases. Return the meaningful remainder and state the shortfall instead of padding.
- Hard-exclude only known broken cases, duplicate IDs or canonical URLs, category/search/home pages, clearly unrelated `off-target` items, unauthorized video-only cases, and near-duplicate variants that would crowd out discovery. Keep roughly at most three cases per source and one or two per near-duplicate family. A platform mismatch may remain as a visual reference when labeled `reference-only`; it is not reusable target-platform code.
- Treat soft-singular wording such as `帮我找一个`, `推荐一个`, `给我一个最像的`, or `选一个最合适的` as a request for a recommendation, not as a one-result limit. Keep the default fifteen-case comparison, put the strongest candidate first or in a separate `最推荐` block, and still show the other meaningful candidates. Reduce the requested count to one only when the user explicitly says an exclusive constraint such as `只要一个，不要其他候选`; this changes the count but does not authorize skipping the visible quick pass.
...
- Link directly to each public item or demo. For an `official-media` record, use its `preview_url` as `观看动效（未实时复核）` in the fuzzy pass and keep the item `url` separately as `来源页`; remove the qualifier only after current live observation. Never send an empty wrapper as the watch link. Do not put a category, search, collection, or homepage in the case list; label those separately as `继续探索入口`.
...
- Label the pass `快速模糊初筛，尚未实时视觉复核` and state that quick tiers are discovery labels, not formal `exact` judgments.
- Interpret `换一批` as the next fifteen unseen fuzzy candidates by default. Accept a requested size up to twenty and exclude every case ID and canonical item URL already shown in the task.

If the user says `只要快速结果`, stop after the quick pass. Otherwise immediately continue visual deep matching in the same running task when concrete item links and visual tools are available; the user does not need to reply before it starts. Deliver the quick list as a progress message rather than ending the task and promising hidden background work. Run expensive page checks and captures in interruptible batches of roughly three to five so feedback can reprioritize the queue between candidates. Skip the visible quick pass only when the current user request explicitly contains `直接深度匹配`; no inferred intent or alternative wording is sufficient. Even then, still create a bounded text-recalled candidate pool before live checking.
