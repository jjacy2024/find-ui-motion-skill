...
When the exit condition is a request to see examples, stop the questionnaire. A generic request to see examples does not authorize video-case search. Read [retrieval-ladder.md](retrieval-ladder.md), run its quick-mode full-index fuzzy scan, and prioritize code-backed or runtime-backed cases that can be implemented on the target platform. Use `quick_tier=strong | related | exploratory` for the quick list; style and feeling preferences rank candidates without becoming all-keyword requirements. Follow `external_search.decision`: skip external work for `skip`, show local results and ask before `offer`, and use a labeled bounded Web supplement only for `required`. Keep that supplement code-first while `video_case_search_authorized=false`. Resolve concrete item links from metadata and show about fifteen sparse fuzzy quick links by default or at most twenty when explicitly requested. Treat soft-singular wording such as `帮我找一个` as recommendation intent: put one recommendation first and keep the other meaningful references. Reduce below fifteen only after the full fuzzy scan, and bilingual expansion when fewer than ten remain, no strong candidate exists, or the core behavior is still missing, cannot supply enough unique candidates; return every meaningful remainder and state the shortfall. Label the pass as not currently live-verified. Skip this visible quick pass only when the current request explicitly contains `直接深度匹配`. Ask whether the user wants a separately labeled video supplement only when that code-first pool is insufficient, and do not start it before confirmation. Then immediately continue through [visual-deep-match.md](visual-deep-match.md) in the same task unless the user asks for quick results only; feedback reprioritizes the queue rather than starting it. Read [source-preview.md](source-preview.md) only when capture or explicit side-by-side comparison is needed. Do not respond with text-only cards or generic source-labeled shapes when real item links are available.
