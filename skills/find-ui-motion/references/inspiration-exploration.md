...
When the exit condition is a request to see examples, stop the questionnaire. Read [retrieval-ladder.md](retrieval-ladder.md), exhaust its local stages, and use a labeled bounded Web supplement only when its deterministic gate recommends one. Resolve concrete item links and show exactly eight eligible sparse quick links by default or at most ten when explicitly requested. Reduce below eight only when fewer unique, relevant, accessible item cases satisfy the current brief; return every eligible remainder and state the shortfall. Then continue through [visual-deep-match.md](visual-deep-match.md) unless the user asks for quick results only. Read [source-preview.md](source-preview.md) only when capture or explicit side-by-side comparison is needed. Do not respond with text-only cards or generic source-labeled shapes when real item links are available.
