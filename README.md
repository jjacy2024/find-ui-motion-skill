...
- 精确搜索、灵感探索和参考重建三种工作流。
- 内置 24 个来源、3671 条经证据门槛筛选的具体案例。
- 按“本地类目 → 3671 条全库模糊检索 → 按需本地同义词”确定性升级，仅在覆盖缺口时建议一次有标记的外网补充。
...
- 快速阶段默认给出约 15 个 `strong / related / exploratory` 模糊候选；深度匹配仍以 8 个严格结果为目标。
- 初筛交付后立即在同一任务内按 3–5 个案例的小批次继续深度匹配，用户反馈会抢占式重排待检查队列。
...
- 自动分页、跨批次去重；未指定时“换一批”默认给出下 15 个未展示的模糊候选。
